...
def enrollment(request, slug):
    ...
    if created:
        ...
        messages.info(request, 'Voce já está incrito no curso')
...
def lesson(request, slug, id):
    course = request.course
    lesson = get_object_or_404(Lesson, pk=id, course=course)
...
def material(request, slug, id):
    course = request.course
    material = get_object_or_404(Material, pk=id, lesson__course=course)
